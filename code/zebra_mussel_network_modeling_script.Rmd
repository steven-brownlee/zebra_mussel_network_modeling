...
write_sf(edge_compilation, 'outputs/network_components/edge_collection.gpkg')



```

# Point of origin generator
...
canada_filtered <- canada_metro_areas %>% select(CMANAME, PRNAME) %>%
  rename(metro_name = PRNAME, prov_state_name = CMANAME) %>% 
...
hex_centroids <- read_sf('/home/steven/Downloads/centroids.gpkg')
...
```{r}
# nb: something wrong with hex id transposition, check
...
edge_collection<- read_sf('outputs/network_components/edge_collection.gpkg') %>% 
   select(hex_rowid_1, hex_rowid_2) %>% 
   rename(from = hex_rowid_1, to = hex_rowid_2)

edge_collection$from <- as.character(edge_collection$from)

edge_collection$to <- as.character(edge_collection$to)

node_collection <- read_sf('outputs/network_components/centroids.gpkg') %>% 
  select(hex_rowid) %>% 
...
edge_collection_test <- edge_collection %>% select(geom)

edge_collection_test <- distinct(edge_collection_test)

edge_collection_test <- edge_collection_test %>% rowid_to_column('edge_id')

test <- edge_collection_test %>% filter(edge_id == 238614)

buff <- st_buffer(test, 20)

int <- st_intersection(node_collection, buff) %>% select(index) %>% st_drop_geometry()

nrow(int)



edge_corrector <- function(target) {
  
  new_edges <- tibble(from = character(),
                    to = character(),
                    geom = st_sfc(crs = 'ESRI:102008'))
  
  for (i in target) {
  ival <- eval(i)
  edge_intermediate <- edge_collection_test %>% filter(edge_id == ival)
  buff <- st_buffer(edge_intermediate, 20)
  int <- st_intersection(node_collection, buff) %>% select(index) %>% 
    st_drop_geometry() %>% 
    deframe()
  edge_intermediate <- edge_intermediate %>% mutate(from = as.character(int[1]), to = as.character(int[2]))
  new_edges <- bind_rows(new_edges, edge_intermediate)
...
corrected_edges <- future_map(2:518769, ~edge_corrector(.x)) %>% bind_rows()

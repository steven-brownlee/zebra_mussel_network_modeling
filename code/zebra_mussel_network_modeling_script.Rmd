...
edge_sfnetwork$from <- as.character(edge_sfnetwork$from)
 ...
 int_path <- st_network_paths(network, 
                              from = 228, 
                              to = destination, 
                              weights = 'weight')
 
nodes <- as_tibble(unlist(int_path$node_paths)) %>% 
      rename(index = value)
int_nodes$index <- as.character(int_nodes$index)
int_node_spatial <- inner_join(node_sfnetwork, int_nodes, by = 'index')

int_node_spatial <- int_node_spatial %>% rowid_to_column('node_index') %>% select(-index)

r1 <- route_collection %>% filter(path_id == 783)

r1 <- r1$geom

r1_sample <- st_line_sample(r1, density = 2000)
...
outer_path_collection_template <- tibble(path_id = numeric(),
                                   origin = character(),
...
for (i in 1:70) {
  ...
  lake_route_collection <- future_map(1:10642, 
                                      ~router(.x, destination = destination)) %>% 
    bind_rows() %>% 
    st_as_sf()
  
  outer_path_collection_template <- bind_rows(outer_path_collection_template,
                                              inner_path_collection_template)
}

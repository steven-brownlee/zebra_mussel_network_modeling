...
spatializer <- function(table, prefix, dist_raster, origins,
      ...
      for (j in 1:20) {
        ...
        interior_final <- interior_final %>%  mutate(x = sf::st_coordinates(.)[,1],
         y = sf::st_coordinates(.)[,2],) %>% 
          st_drop_geometry()
...
fresno <- test_grouped %>% filter(metro_name == 'Fresno')
  
oklahoma_city <- test_grouped %>% filter(metro_name == 'Oklahoma City')
  
salt_lake_city <- test_grouped %>% filter(metro_name == 'Salt Lake City')

halifax <- test_grouped %>% filter(metro_name == 'Halifax')
  
roanoke <- test_grouped %>% filter(metro_name == 'Roanoke') 
  
ann_arbor <- test_grouped %>% filter(metro_name == 'Ann Arbor')

sample_origins <- bind_rows(fresno, oklahoma_city, salt_lake_city, halifax,
                            roanoke, ann_arbor)
...
slice_example <- ggplot() +  
  geom_line(sample_origins, mapping = aes(x = route_index, y = origin_id_fixed, 
                                           colour = metro_name),
 ...
 paletteer::scale_color_paletteer_d("fishualize::Oncorhynchus_gorbuscha") +
  ...
  new_scale_color() +
  geom_point(sample_filtered, mapping = aes(x = route_index, y = origin_id_fixed,

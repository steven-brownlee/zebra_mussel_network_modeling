...
edging <- function(row_subset) {
  
  pairwise_distance_table <- tibble(hex_rowid_a = numeric(),
                                 hex_rowid_b = numeric(),
                                 comb_hex_id = character(), 
                                 distance = numeric(),
                                 x_1 = numeric(),
                                 y_1 = numeric(),
                                 x_2 = numeric(),
  ...
  for (y in 1:518769) {
    yval = eval(y)
    coord_2 <- hex_centroids %>% filter(hex_rowid == yval)
    pair_dist <- st_distance(coord_1, coord_2)
    combined_hex_rowid <- paste0(as.character(coord_1$hex_rowid), '_', as.character(coord_2$hex_rowid))
    interior_table <- tibble(hex_rowid_a = coord_1$hex_rowid,
                                 hex_rowid_b = coord_2$hex_rowid,
                                 comb_hex_id = combined_hex_rowid, 
                                 distance = as.numeric(pair_dist),
                                 x_1 = coord_1$x,
                                 y_1 = coord_1$y,
                                 x_2 = coord_2$x,
                                 y_2 = coord_2$y)
    pairwise_distance_table <- rbind(pairwise_distance_table, interior_table)
  }
  }
  
  pairwise_distance_table <- pairwise_distance_table %>% filter(distance == 2000)
  return(pairwise_distance_table)
}

edge_set <- future_map(1:518769, ~edging(.x))
...
for (x in 1:5) {
  ...
  for (y in 1:200) {
    ...
    interior_table <- tibble(hex_rowid_a = coord_1$hex_rowid,
                                 ...
                                 y_2 = coord_2$y)
    pairwise_distance_table <- rbind(pairwise_distance_table, interior_table)
    print(paste('Completed C1', x, 'C2', y))
  }
  print(paste('Completed:', x))
}

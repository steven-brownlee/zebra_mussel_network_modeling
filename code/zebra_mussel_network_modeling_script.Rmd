...
zm_occ <- zm_occurrences %>% mutate(occ_val = 1) %>% select(occ_val) %>% 
  vect()

template_rast <- north_america_template_raster %>% rast()

test_rast <- rasterize(zm_occ_raster, template_rast, field = 'occ_val') 

euclidean_distance <- terra::distance(test_rast)

euclidean_distance <- mask(euclidean_distance, mask = land_vect)

writeRaster(distance_test, 'processed/euclidean_distance_from_zm.tif')
...
spatializer <- function(table, prefix, dist_raster, origins) {
      ...
      for (j in 1:20) {
        ...
        interior_selection <- table_groups[[j_val]] %>% select(-index)
        
        interior_joined <- left_join(interior_selection, origins, 
                               by = 'origin_id') %>% select(-index) %>% 
          st_as_sf(coords = c('x', 'y'), 
             crs = st_crs('ESRI:102008'))
        
        interior_extraction <- st_extract(dist_raster, interior_joined,
                                    FUN = 'mean') %>% 
          distinct()
      ...
      path <- paste0('outputs/network_components/routes/', eval(prefix), 
      '/', eval(prefix), '_route_', x, '.gpkg') 
      write_sf(final_template, path)
...
for (x in 1:length(groups)) {
  ...
  if (nrow(ok_table > 0)) {
      spatializer(table = metro_table, prefix = 'metro', dist_raster = distance_raster,
              origins = origins_matched)
  } else {
...
for (x in 1:length(groups)) {
  ...
  if (nrow(ok_table > 0)) {
      spatializer(table = lake_table, prefix = 'lake', dist_raster = distance_raster,
              origins = origins_matched)
  } else {
    print(paste0('No data for ', x, ', skipping!'))
  }
...
for (x in 1:length(groups)) {
  ...
  if (nrow(ok_table > 0)) {
      spatializer(table = ok_table, prefix = 'okanagan', dist_raster = distance_raster,
              origins = origins_matched)
  } else {
    print(paste0('No data for ', x, ', skipping!'))
  }

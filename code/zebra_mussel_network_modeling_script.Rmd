...
for (i in 1:200)  {
  hexagon_slice <- read_csv(paste0('data_directory/final_osm/final_osm_chunks/osm_row_table_chunk_', i, '.csv'),
                             show_col_types = FALSE) %>% 
    group_by(rowid)
  hexagon_summarized <- hexagon_slice %>% summarize_at(c('max_speed', 'type', 'lanes'),
                     max, na.rm = TRUE)
  osm_hexagon_append <- bind_rows(osm_hexagon_append, hexagon_summarized)
  print(paste('Completed:', i))
}


osm_hexagon_summarized <- 
        summarize_at(c('max_speed', 'type', 'lanes'),
                     max, na.rm = TRUE)

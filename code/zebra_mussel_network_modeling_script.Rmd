...
for (i in 1:2000) {
  ...
  write_sf(osm_processed, paste0('scratch/filtered_outdir/osm_filtered_data_', i, '.gpkg'))
  print(paste('Completed:', i))
}
...
osm_part_one <- osm_data_list[1:710]

osm_part_two <- osm_data_list[711:1420]

osm_part_three <- osm_data_list[1420:2000]




big_reader <- function(gpkg_list) {
  p  <- progressr::progressor(along = gpkg_list)
  osm_sf_list <- future_map(gpkg_list, 
                                globals = c(gpkg_list = gpkg_list),
    ~{ p()
    read_sf(.x)})
  return(osm_sf_list)
}

with_progress({osm_combined_one <- big_reader(osm_part_one)})

with_progress({osm_combined_two <- big_reader(osm_part_two)})

with_progress({osm_combined_three <- big_reader(osm_part_three)})


combiner <- function(sf_list) {
  p  <- progressr::progressor(along = sf_list)
  osm_combined <- future_map(sf_list, 
                                globals = c(sf_list = sf_list),
    ~{ p()
    bind_rows(.x)})
  return(osm_combined)
}
...
with_progress({osm_sf_combined_p1 <- combiner(osm_part_one)})

with_progress({osm_sf_combined_p2 <- combiner(osm_part_two)})

with_progress({osm_sf_combined_p3 <- combiner(osm_part_three)})

write_sf(osm_sf_combined_p1, 'data_directory/osm_combined_p1.gpkg')

write_sf(osm_sf_combined_p2, 'data_directory/osm_combined_p2.gpkg')

write_sf(osm_sf_combined_p3, 'data_directory/osm_combined_p3.gpkg')
...
summarizer <- function(id) {
  osm_selected <- read_sf(paste('data_directory/final_osm/osm_hexagon_aligned_', id, '.gpkg'))
  hexagon_selected <- read_sf(paste('data_directory/final_hexagon/osm_hexagon_aligned_', id, '.gpkg'))
...
for (i in 1:40) {
  osm_hexagon_selection <- summarizer(i)
  write_sf(osm_hexagon_selection, paste0('data_directory/osm_hexagon_selection_', i, '.gpkg')
}

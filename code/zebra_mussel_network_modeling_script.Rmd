...
for (i in 1:167) {
  with_progress({osm_processed <- osm_processor(osm_slice_list[i])})
  osm_processed <- bind_rows(osm_processed)
  write_csv(osm_processed, paste0('osm_point_table_processed_', i ,'.csv'))
}

for (i in 168:334) {
  with_progress({osm_processed <- osm_processor(osm_slice_list[i])})
  write_csv(paste0('osm_point_table_processed_', i ,'.csv'))
}

for (i in 334:500) {
  with_progress({osm_processed <- osm_processor(osm_slice_list[i])})         write_csv(paste0('osm_point_table_processed_', i ,'.csv'))
}

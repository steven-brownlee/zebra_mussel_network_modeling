...
router <- function(row_subset, destination) {
  ...
  inner_path_collection_template <- tibble(path_id = numeric(),
                                origin = character(),
  ...
  if (length(unlist(int_path$node_paths)) > 0) {
    ...
    path_collection_int <- tibble(path_id = ival,
                          ...
                          geom = path_edges)
    inner_path_collection_template <- bind_rows(inner_path_collection_template, path_collection_int)
  } 
}
  return(inner_path_collection_template)
}
...
route_tabulator <- function(row_subset, route_collection) {

s_val <- eval(row_subset)

eval_row <- route_collection %>% filter(row_counter == s_val)

  suppressWarnings(
  route_interior  <- eval_row %>% 
  select(-row_counter) %>% 
  st_cast('POINT') %>% 
  rowid_to_column('route_index') %>% 
  arrange(desc(route_index)) %>% 
  rowid_to_column('route_index_fixed') %>% 
  select(-route_index) %>% 
  rename(route_index = route_index_fixed) %>%
  mutate(path_id = eval_row$path_id,
         origin = eval_row$origin,
         destination = eval_row$destination)
)

return(route_interior)
}
...
edge_sfnetwork <- read_sf('outputs/network_components/edge_sfnetwork_routing.gpkg')
...
groups <- split(1:101612, ceiling(seq_along(1:101612)/25))

###


for (i in 1:nrow(metro_dest)) {
  
  d_val = eval(i)
  
  destination <- metro_dest %>% filter(destination_id == d_val)
  
  ###
  
  outer_route_analysis_template <- tibble(route_index = numeric(),
                                  path_id = numeric(),
                                  origin = character(),
                                  destination = character(),
                                  geom = st_sfc(crs = 'ESRI:102008')) %>% 
  ...
  index <- unlist(groups[j])
    
  metro_route_collection <- future_map(index, 
                                      ~router(.x, destination = destination)) %>% 
    bind_rows() %>% 
    st_as_sf() %>% 
    rowid_to_column('row_counter')
  

  
  metro_route_points <- future_map(1:nrow(metro_route_collection),
                                  ~route_tabulator(.x,
                                                   route_collection = 
                                                     metro_route_collection)) %>% 
    bind_rows() 
  
  outer_route_analysis_template <- bind_rows(outer_route_analysis_template, 
                                             metro_route_points)
  
  print(paste('Completed interior group:', j, 'of', length(groups)))
    
  }
  
  filename <- paste0('outputs/network_components/routes/metroanagan/metro_route_', i, '.gpkg')
  
  write_sf(outer_route_analysis_template, filename)
  
print(paste('Completed:', i, 'of', nrow(metro_dest) ))
print('~~~')
}
...
for (i in 1:nrow(lake_dest)) {
  
  d_val = eval(i)
  
  destination <- lake_dest %>% filter(destination_id == d_val)
  ...
  lake_route_collection <- future_map(index, 
                                      ~router(.x, destination = destination)) %>% 
    bind_rows() %>% 
    st_as_sf() %>% 
    rowid_to_column('row_counter')
  ...
  lake_route_points <- future_map(1:nrow(lake_route_collection),
                                  ~route_tabulator(.x,
                                                   route_collection = 
                                                     lake_route_collection)) %>% 
    bind_rows() 
  ...
  filename <- paste0('outputs/network_components/routes/lake/lake_route_', i, '.gpkg')
  
  write_sf(outer_route_analysis_template, filename)
  
print(paste('Completed:', i, 'of', nrow(lake_dest) ))
print('~~~')
}
...
for (i in 1:nrow(ok_dest)) {
  
  d_val = eval(i)
  
  destination <- ok_dest %>% filter(destination_id == d_val)
  ...
  ok_route_collection <- future_map(index, 
                                      ~router(.x, destination = destination)) %>% 
    bind_rows() %>% 
    st_as_sf() %>% 
    rowid_to_column('row_counter')
  ...
  ok_route_points <- future_map(1:nrow(ok_route_collection),
                                  ~route_tabulator(.x,
                                                   route_collection = 
                                                     ok_route_collection)) %>% 
    bind_rows() 
  ...
  filename <- paste0('outputs/network_components/routes/okanagan/ok_route_', i, '.gpkg')
  
  write_sf(outer_route_analysis_template, filename)
  
print(paste('Completed:', i, 'of', nrow(ok_dest) ))
print('~~~')
}
...
test <- split(d, ceiling(seq_along(d)/1000))

for (i in 1:10) {
  inside_index <- group_1[i]
  outside_index <- group_2[i]
  route_subset <- route_collection %>% 
    filter(collection_id >= inside_index & collection_id < outside_index)
  write_sf(route_subset, paste0('outputs/network_components/path_folder/summerland_paths/path_subset_', i, '.gpkg'))
  print(paste('Completed:', i))
}

write_sf(route_collection, 'outputs/network_components/summerland_route_collection.gpkg')

route_collection <- read_sf('outputs/network_components/summerland_route_collection.gpkg')

route_collection_aspatial <- route_collection %>% st_drop_geometry()

write_rds(route_collection_aspatial, 'outputs/network_components/route_collection_aspatial.rds')

#route_1 <- route_collection %>% filter(path_id < 300)

#write_sf(route_1, 'outputs/network_components/route_testbed.gpkg')
...
# Extract values from mussel buffer
...
route_collection <- route_collection %>% rowid_to_column('collection_id')

zm_buffer_raster <- st_rasterize(zm_buffer)

route_summarizer <- function(row_subset) {

  route_summaries <- tibble(path_id = numeric(),
                          origin = character(),
                          destination = character(),
                          route_max = numeric(),
                          route_min = numeric(),
  ...
  ival  = eval(i)
  r_subset <- route_collection %>% filter(collection_id == ival)
  r_sample <- st_cast(r_subset, 'POINT')
  r_extract_df <- st_extract(zm_buffer_raster, r_sample) %>%
    st_drop_geometry()
  r_extract_undf <- r_extract_df %>%
    st_drop_geometry() %>% 
    deframe() %>% 
    na.omit()
  r_max <- max(r_extract_undf)
  r_avg <- mean(r_extract_undf)
  r_min <- min(r_extract_undf)
  r_sd <- st(r_extract_undf)
  r_prop_na <- sum(is.na(r_extract_df$buffer_dist))
  r_sample_size = nrow(r_extract_df)
    r_dist <- st_length(r_subset) %>%
    as.numeric() 
  r_dist = r_dist*0.001
  ...
  ###
    ...
    route_summaries_int <- tibble(path_id = r_subset$path_id,
                          origin = r_subset$origin,
                          destination = r_subset$destination,
                          route_max = r_max,
                          route_min = r_min,
                          route_avg = r_avg,
                          route_sd = r_sd,
                          prop_na = r_prop_na,
                          sample_size = r_sample_size,
                          length_path = r_dist,
                          )
  ###
    
  route_summaries <- bind_rows(route_summaries, route_summaries_int)
}
  return(route_summaries)

}

route_summaries_outer <- tibble(path_id = numeric(),
                          origin = character(),
                          destination = character(),
                          route_max = numeric(),
                          route_min = numeric(),
...
for (i in 1:10) {
  ival = eval(i)
  route_subset <- read_sf(paste0('outputs/network_components/path_folder/summerland_paths/path_subset_', i, '.gpkg'))
  length_sample <- nrow(route_subset)
  route_summary_part <- future_map(length_sample, ~route_summarizer(.x)) %>% bind_rows()
  route_summaries_outer <- bind_rows(route_summaries_outer, route_summary_part)
  print(paste('Completed:', i))
}

...
qgis_wrapper <- function(group_iterator, osm_slice) {
        osm_slice <- osm_slice %>% filter(group_number == group_iterator)
...
osm_processor <- function(group_slice) {
  ...
  group_vector <- unique(interior_osm$group_number)
  p <- progressr::progressor(steps = group_length)
  result <- future_map(group_vector, globals = c(interior_osm = interior_osm),
    ~{ p()
...
with_progress({osm_test <- osm_processor(test)})

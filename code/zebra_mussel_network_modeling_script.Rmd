...
hex_id_array <- hex_centroids %>% 
  select(hex_rowid, x, y) %>% 
  rename(hex_rowid_2 = hex_rowid, x_2 = x, y_2 = y)

hex_id_array_aspatial <- hex_id_array %>% st_drop_geometry()
...
edge_generator <- function(row_subset) {
  ...
  for (x in row_subset) {
    ...
    distance_array <- as_tibble(st_is_within_distance(coord_1, hex_centroids, 
                                                      dist = 2000, sparse = FALSE))
    distance_array <- pivot_longer(distance_array,
                           everything(),
                           names_to = 'prefix', 
    ...
    distance_array <- bind_cols(hex_id_array, distance_array)
    
    distance_array <-  distance_array %>% filter(within_threshold == TRUE) %>%
      mutate(hex_rowid_1 = xval, 
         x_1 = coord_1$x,
         y_1 = coord_1$y) %>% 
      mutate(comb_hex_rowid = paste0(as.character(hex_rowid_1), '_', as.character(hex_rowid_2))) %>%
      select(hex_rowid_1, x_1, y_1, hex_rowid_2, x_2, y_2, comb_hex_rowid)
    distance_array <- rbind(distance_array_template, distance_array)
     p()
  }
  return(distance_array)
}

inside_list = c(1, 25000, 50000, 75000, 100000, 125000, 150000, 175000, 
                200000, 225000, 250000, 275000, 300000, 325000, 350000, 
                375000, 400000, 425000, 450000, 475000, 500000)

outside_list= c(25000, 50000, 75000, 100000, 125000, 150000, 175000, 200000, 
                225000, 250000, 275000, 300000, 325000, 350000, 375000, 400000, 
                425000, 450000, 475000, 500000, 518769)

outer_process <- function(ind_1, ind_2) {

for (x in ind_1:ind_2) {
  index_1 <- inside_list[x]
  index_2 <- outside_list[x]
  comb_index <- index_1:index_2
  p <- progressr::progressor(along = comb_index)
  edge_set <- future_map(1:250000, ~{ p()
    edge_generator(.x)})
  edge_collection <- bind_rows(edge_set)
  write_rds(edge_collection, 
            paste0('outputs/network_components/edge_folder/', x, 
                   '_edge_collection.rds'))
}
}

# Computer 1:
  
with_progress({outer_process(1, 7)})

# Computer 2:
  
with_progress({outer_process(7, 14)})

# Computer 3:

with_progress({outer_process(14, 21)})

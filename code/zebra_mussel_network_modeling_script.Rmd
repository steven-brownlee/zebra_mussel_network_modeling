...
for (i in 1:length(okanagan_files)) {
  ...
  intermediate <- okanagan_files[ival]
  intermediate <- read_rds(intermediate)
  intermediate <- intermediate %>% select(x,)
  pt_template <- bind_rows(pt_template, intermediate)
  print(paste('Completed:', ival))
}

grid <- st_make_grid(qgis_bbox, cellsize = 500, 
...
write_stars(grid, 'processed/template_grid.tif')

grid_clipped <- st_crop(grid, land)

write_stars(grid_clipped, 'processed/template_grid_clipped.tif')

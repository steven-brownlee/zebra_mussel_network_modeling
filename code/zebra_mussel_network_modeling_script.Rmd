...
```{r}
zm_occurrences <- read_sf('data_directory/usgs_nas_dreissenid_occurrences.shp')

north_america_bbox <- read_sf('data_directory/qgis_north_america_bbox.shp')

us_major_cities_centroid <- read_sf('data_directory/cec_populated_places.shp')


land <- read_sf('data_directory/qgis_cec_coastline.shp') %>% st_intersection(north_america_bbox)

large_lakes <- read_sf('data_directory/cec_lakes_shapefile.shp') %>% st_intersection(north_america_bbox)
...
overlay_grid <- st_make_grid(north_america_bbox, cellsize = 10000, what = 'polygons',
                             square = FALSE) %>% st_as_sf()

write_sf(overlay_grid, 'scratch/10km_hexagon_grid.gpkg')

# Nb for users: this line invokes code from the QGIS 
# 'extract by location' function. It is significantly
# slower than its GUI counterpart, unfortunately. Users
# may therefore want to step away from this code and run
# it directly in QGIS.
...
# In case you ran the function in QGIS:

# overlay_selection <- read_sf('scratch/10km_hexagon_selection.gpkg')

overlay_selection <- overlay_selection %>% rowid_to_column('uid') %>% select(uid)

write_sf(overlay_selection, 'data_directory/10km_hexagon_selection.gpkg', driver = 'GPKG')
...
test_filter <- overlay_selection %>% filter(uid == 19060)

oe_download_directory('/home/steven/Documents/common/thesis/ch2/gis_data/downloads/open_street_map_r')
  
write_sf(osm_grid, '/home/steven/Documents/common/thesis/ch2/gis_data/scratch/test_selection.shp')

test_bbox <- st_bbox(test_filter)

test_query <- opq(bbox = c(test_bbox[1], test_bbox[2], test_bbox[3], test_bbox[4])) %>% osmdata_sf()

test_osmdata <- osmdata_sf(test_query)

test <- oe_get(place = test_filter, boundary = test_filter)

...
test <- read_sf(osm_slice_list[447]) %>% rowid_to_column('uid')

##

divided_dataset <- tibble(x = seq(1:4254041))
...
for (i in 1:100)  {
  na_slice <- test %>% 
    filter(uid >= sequence_start[i] & uid <= sequence_end[i])
  write_sf(na_slice, paste0('scratch/osm_point_table_processed_447_', i, '.gpkg'))
  print(paste('Completed:', i))
}


osm_447_list = list.files('/home/steven/Documents/common/thesis/ch2/gis_data/', 
                      pattern = 'osm_point_table_processed_447_*',
                      full.names = TRUE,
                      recursive = TRUE)
for (i in 1:100) {
  with_progress({osm_processed <- outer_process(osm_447_list[i])})    
  osm_processed <- bind_rows(osm_processed)
  print(paste('Completed:', i))
  write_csv(osm_processed, paste0('scratch/point_outdir/osm_point_table_processed_447_', i ,'.csv'))
}


###

with_progress({osm_processed <- outer_process(osm_slice_list[447])})  

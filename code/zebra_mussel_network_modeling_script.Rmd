...
na_slice <- read_sf('scratch/group_slice_447.gpkg')
...
for (i in 1:length(osm_data_list))  {
 ...
 osm_point_intermediate <- osm_point_intermediate %>% 
   mutate(lon = sf::st_coordinates(.)[,1],
          lat = sf::st_coordinates(.)[,2]) %>% 
    st_drop_geometry()
 write_csv(osm_point_intermediate, paste0('scratch/osm_points_organized_', i, '.csv'))
...
osm_filter <- function(input_table) {
  ...
  combined_osm_table <- input_table %>% filter(!group_id %in% c(NA)) %>% 
  ...
  osm_filtered_data <- osm_filtered_data %>% select(point_id, osm_id.x, 
                                                    group_id.x, name, lanes, 
                                                    max_speed) %>% 
    rename()
...
for (i in 1:545) {
  ...
  write_sf(osm_processed, paste0('scratch/osm_filtered_data_', i, '.gpkg'))
  print(paste('Completed:', i))
}
...
osm_combined <- osm_data_list %>% map(read_sf)

osm_combined <- do.call('rbind', osm_combined)

write_sf(osm_combined, 'data_directory/osm_points_combined.gpkg')

hexagon_north_america <- read_sf('data_directory/2km_hexagon_selection_grouped.gpkg')

for (i in 1:40) {
  osm_hexagon_aligned <- osm_combined %>% filter(group_id = i)
  write_sf(osm_hexagon_aligned, paste('data_directory/final_osm/osm_hexagon_aligned_', i, '.gpkg'))
  print(paste('Completed:', i))
}

for (i in 1:40) {
  hexagon_subset <- hexagon_north_america %>% filter(group_id = i)
  write_sf(hexagon_subset, paste('data_directory/final_hexagon/osm_hexagon_aligned_', i, '.gpkg'))
  print(paste('Completed:', i))
}
...
inner_summarizer <- function(id) {
  osm_selected <- read_sf(paste('data_directory/final_osm/osm_hexagon_aligned_', i, '.gpkg'))
  hexagon_selected <- read_sf(paste('data_directory/final_hexagon/osm_hexagon_aligned_', i, '.gpkg'))
  joined_points <- st_join(hexagon_selected, osm_selected,
                               left = TRUE,
                               join = st_intersects) %>% 
        st_drop_geometry()
...
osm_summarized <- map(1:40, osm_summarizer)

write_sf(osm_summarized, 'data_directory/osm_summarized.gpkg')

...
edge_collection <-  edge_compilation %>% 
...
duplicate_list <- st_equals(edge_collection, retain_unique = TRUE)

edge_collection <- edge_collection[-unlist(duplicate_list),]

edge_collection <- edge_collection %>% rowid_to_column('unique_id')

#write_sf(edge_collection, '/home/steven/Downloads/edge_collection.gpkg')

midpoint_generator <- function(row_subset) {
  
  midpoint_collection <- tibble(id = numeric(),
                    geom = st_sfc(crs = 'ESRI:102008'))
  
  for (i in row_subset) {
    ival = eval(i)
    int_filter <- edge_collection %>% filter(unique_id == ival)
    int_midpoint <- st_sample(test, size = 1, type = 'regular') %>%
      st_cast("POINT") %>% 
      st_as_sf() %>% 
      mutate(id = ival) %>% 
      rename(geom = x)
...
midpoint_collection <- future_map(midpoint_generator(1:518769)) %>% bind_rows()
...
network <- sfnetwork(node_collection, edge_collection, node_key = 'index', 
                     force = TRUE)

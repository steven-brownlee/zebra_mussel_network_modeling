...
qgis_wrapper <- function(group_iterator, osm_slice) {
        osm_segmented <- qgis_run_algorithm("native:splitlinesbylength",
                     INPUT = filter(osm_slice, 
                                    group_number == group_iterator),
...
osm_processor <- function(group_slice) {
  interior_osm <- read_sf(group_slice)
  group_length <- length(unique(interior_osm$group_number))
  group_vector <- 1:group_length
  p <- progressr::progressor(steps = group_length)
  result <- future_map(group_vector, ~{
    p()
    qgis_wrapper(.x, osm_slice = interior_osm)})
  return(result)
}
...
for (i in 1:167) {
  iterate_slice <- osm_slice_list[i]
  with_progress({osm_processed <- osm_processor(osm_slice_list[1])})
  write_csv(paste0('osm_point_table_processed', i ,'.csv'))
}
...
for (i in 168:334) {
  iterate_slice <- osm_slice_list[i]
  with_progress({osm_processed <- osm_processor(osm_slice_list[1])})         write_csv(paste0('osm_point_table_processed', i ,'.csv'))
}
